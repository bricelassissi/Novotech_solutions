...
class AccountController extends Controller
{
    ...
    public function loginClient() {
        return view('front.compte.connexion-client');
    }
    ...
    public function clientAuthentification(Request $request) {
        ...
        if ($validator->passes()) {
            ...
            if(Auth::attempt(['email' => $request->email, 'password' => $request->motDePasse])) {
                ...
                $baseUrl = url('/');
                return redirect()->to($baseUrl . '/artisan/detail/' . $artisanId);
                // return redirect()->to('http://localhost:8000/artisan/detail/' . $request->artisanId);
            } else {
                return redirect()->route('client.login')->with('error','Attention, votre identifiant ou votre mot de passe est incorrect.');
            }
